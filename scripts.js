class Cuttt {
  ...
  onLeave() {
    ...
    const points = temp.filter(
      (t) => t.x >= 0 && t.x <= offsetWidth && t.y >= 0 && t.y <= offsetHeight
    );
    const lineIndex = [
      this.getLineIndex(this.el2, points[0]),
      this.getLineIndex(this.el2, points[1]),
    ];
    ...
    cp1[clipIndex[0][0]] = [
      (points[0].x / offsetWidth) * 100,
      (points[0].y / offsetHeight) * 100,
    ];
    cp1[clipIndex[0][1]] = [
      (points[1].x / offsetWidth) * 100,
      (points[1].y / offsetHeight) * 100,
    ];
    cp2[clipIndex[1][0]] = [
      (points[0].x / offsetWidth) * 100,
      (points[0].y / offsetHeight) * 100,
    ...
    timeline.to(
      this.el1,
      { x: -x, y: -y, duration: this.duration, ease: "Power4.easeOut" },
      0
    );
    timeline.to(
      this.el2,
    ...
    timeline.to(
      this.el1,
      { x: 0, y: 0, duration: this.duration * 2, ease: "Power4.easeIn" },
      this.duration * 2
    );
    timeline.to(
      this.el2,
...
function changeChildren(type) {
  let element;
  if (type === "text") {
    element = "<p data-for-cut>Looser</p>";
  } else if (type === "input") {
    element = '<input data-for-cut type="text"></input>';
  } else if (type === "image") {
    element =
      '<img data-for-cut src="https://images.unsplash.com/photo-1506014299253-3725319c0f69?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80">';
  }
...
function initInputText() {
  container.children[0].value = container.children[1].value =
    "You will want to cut this...";
